...
class Instant_Articles_Post {
    ...
    public function to_instant_article() {
        ...
        //TODO: add_recirculation_ads meta property
        if (defined('FBIA_PLACEMENT_ID_RECIRCULATION')) {
            $id = (FBIA_PLACEMENT_ID_RECIRCULATION);
            $this->instant_article->addMetaProperty( 'fb:op-recirculation-ads', 'placement_id='.$id );
        }
    ...
    public function add_ads_from_settings() {
        ...
        case 'fan':
            if (defined('FBIA_PLACEMENT_ID')) {
                $placement_id = (FBIA_PLACEMENT_ID);
                $ad->withSource(
                    add_query_arg(
                        array(
    ...
    public function add_analytics_from_settings() {
        ...
        if ( isset( $settings_analytics['embed_code_enabled'] ) && ! empty( $settings_analytics['embed_code'] ) ) {
            if (defined('GAP_ID')) {
                $document = new DOMDocument();
                $fragment = $document->createDocumentFragment();
                $valid_html = @$fragment->appendXML( str_replace('{GAP_ID}', GAP_ID,  $settings_analytics['embed_code']) );
        ...
        $document = new DOMDocument();
        $fragment = $document->createDocumentFragment();
        $fixel_script = "<script> !function(f,b,e,v,n,t,s){if(f.fbq)return;n=f.fbq=function(){n.callMethod?  n.callMethod.apply(n,arguments):n.queue.push(arguments)};if(!f._fbq)f._fbq=n; n.push=n;n.loaded=!0;n.version='2.0';n.queue=[];t=b.createElement(e);t.async=!0; t.src=v;s=b.getElementsByTagName(e)[0];s.parentNode.insertBefore(t,s)}(window, document,'script','https://connect.facebook.net/en_US/fbevents.js'); fbq('init', '236800553426598'); fbq('track', 'Lead'); </script>";

        $valid_html = @$fragment->appendXML($fixel_script);
