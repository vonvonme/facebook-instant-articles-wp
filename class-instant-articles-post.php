 ...
 */
	...
	public function to_instant_article() {
		...
		$_old_ia_markup = isset($_GET['ia_markup']) ? $_GET['ia_markup'] : false;
		$_GET['ia_markup'] = '1';
		global $wp_embed;
		$the_content = $wp_embed->autoembed($the_content);
		$the_content = $wp_embed->run_shortcode($the_content);
		$the_content = do_shortcode($the_content);
		if ($_old_ia_markup) {
			$_GET['ia_markup'] = $_old_ia_markup;
		} else {
			unset($_GET['ia_markup']);
		}
		...
		if ( ! has_filter( 'the_content', 'Vonvon\Plugin\attach_rumble_player') && function_exists('Vonvon\Plugin\attach_rumble_player') )
